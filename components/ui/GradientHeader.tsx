/**
 * Reusable GradientHeader Component
 * Can be used across all features for consistent header styling
 */

import React from 'react';
import { View, Text, TouchableOpacity, StyleSheet, Platform } from 'react-native';
import { SafeLinearGradient as LinearGradient } from '@/components/ui/SafeLinearGradient';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import * as Haptics from 'expo-haptics';
import Svg, { Path } from 'react-native-svg';
import { Colors, Spacing, FontSizes, FontWeights } from '@/constants/theme';

interface GradientHeaderProps {
  title: string;
  gradientColors?: [string, string];
  showBackButton?: boolean;
  showCloseButton?: boolean;
  rightElement?: React.ReactNode;
  onBackPress?: () => void;
  onClosePress?: () => void;
}

export default function GradientHeader({ 
  title, 
  gradientColors = [Colors.gradientStart, Colors.gradientEnd],
  showBackButton = true,
  showCloseButton = false,
  rightElement,
  onBackPress,
  onClosePress,
}: GradientHeaderProps) {
  const router = useRouter();

  const handleBackPress = () => {
    if (Platform.OS === 'ios') {
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    }
    
    if (onBackPress) {
      onBackPress();
    } else {
      router.back();
    }
  };

  const handleClosePress = () => {
    if (Platform.OS === 'ios') {
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    }
    
    if (onClosePress) {
      onClosePress();
    } else {
      router.back();
    }
  };

  return (
    <LinearGradient
      colors={gradientColors}
      style={styles.gradient}
      start={{ x: 0, y: 0 }}
      end={{ x: 1, y: 0 }}
    >
      <SafeAreaView edges={['top']} style={styles.safeArea}>
        <View style={styles.header}>
          {/* Back Button with white circle background */}
          {showBackButton && (
            <TouchableOpacity
              style={styles.backButton}
              onPress={handleBackPress}
              activeOpacity={0.6}
            >
              <View style={styles.iconCircle}>
                <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <Path d="M15.13 19.0596H7.13C6.72 19.0596 6.38 18.7196 6.38 18.3096C6.38 17.8996 6.72 17.5596 7.13 17.5596H15.13C17.47 17.5596 19.38 15.6496 19.38 13.3096C19.38 10.9696 17.47 9.05957 15.13 9.05957H4.13C3.72 9.05957 3.38 8.71957 3.38 8.30957C3.38 7.89957 3.72 7.55957 4.13 7.55957H15.13C18.3 7.55957 20.88 10.1396 20.88 13.3096C20.88 16.4796 18.3 19.0596 15.13 19.0596Z" fill={Colors.text}/>
                  <Path d="M6.43006 11.5599C6.24006 11.5599 6.05006 11.4899 5.90006 11.3399L3.34006 8.77988C3.05006 8.48988 3.05006 8.00988 3.34006 7.71988L5.90006 5.15988C6.19006 4.86988 6.67006 4.86988 6.96006 5.15988C7.25006 5.44988 7.25006 5.92988 6.96006 6.21988L4.93006 8.24988L6.96006 10.2799C7.25006 10.5699 7.25006 11.0499 6.96006 11.3399C6.82006 11.4899 6.62006 11.5599 6.43006 11.5599Z" fill={Colors.text}/>
                </Svg>
              </View>
            </TouchableOpacity>
          )}

          {/* Title */}
          <Text style={styles.title}>{title}</Text>

          {/* Right Element or Close Button with white circle background */}
          {showCloseButton ? (
            <TouchableOpacity
              style={styles.closeButton}
              onPress={handleClosePress}
              activeOpacity={0.6}
            >
              <View style={styles.iconCircle}>
                <Svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                  <Path d="M18 6L6 18" stroke={Colors.text} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                  <Path d="M6 6L18 18" stroke={Colors.text} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                </Svg>
              </View>
            </TouchableOpacity>
          ) : rightElement ? (
            <View style={styles.rightElement}>{rightElement}</View>
          ) : (
            showBackButton && <View style={styles.placeholder} />
          )}
        </View>
      </SafeAreaView>
    </LinearGradient>
  );
}

const styles = StyleSheet.create({
  gradient: {
    paddingBottom: 0,
  },
  safeArea: {
    backgroundColor: 'transparent',
  },
  header: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingHorizontal: Spacing.lg,
    paddingTop: Spacing.md, // STANDARD: Matches Rizz page (16px)
    paddingBottom: 0,
  },
  backButton: {
    width: 44, // STANDARD: Matches Rizz page
    height: 44,
    justifyContent: 'center',
    alignItems: 'center',
  },
  closeButton: {
    width: 44, // STANDARD: Matches Rizz page
    height: 44,
    justifyContent: 'center',
    alignItems: 'center',
  },
  iconCircle: {
    width: 44, // STANDARD: Matches Rizz page
    height: 44,
    borderRadius: 22,
    backgroundColor: Colors.background,
    justifyContent: 'center',
    alignItems: 'center',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.1,
    shadowRadius: 4,
    elevation: 3,
  },
  title: {
    flex: 1,
    fontSize: FontSizes.xl,
    fontWeight: FontWeights.bold,
    color: Colors.textWhite,
    textAlign: 'center',
  },
  rightElement: {
    width: 44, // STANDARD: Matches Rizz page
    height: 44,
    justifyContent: 'center',
    alignItems: 'center',
  },
  placeholder: {
    width: 44, // STANDARD: Matches Rizz page
    height: 44,
  },
});
